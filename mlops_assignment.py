...
##CHECKING FOR PR REVIEW AGENT   -*- coding: utf-8 -*-
"""mlops-assignment.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1ymV2wXYjqSVL3bkKoVVU0nHcnh3Sno7Z
...
## **Model Rationale: Why DistilBERT?**

`distilbert-base-cased` is a distilled, lighter version of BERT developed by Hugging Face through a process called knowledge distillation. It retains approximately 97% of BERT's language understanding capabilities while being 40% smaller and 60% faster, making it well-suited for training on Kaggle's free GPU tier within resource and time constraints. The cased variant is chosen specifically for this task because book reviews frequently contain proper nouns, author names, and genre-specific terms where letter case carries semantic meaning. DistilBERT was pre-trained on English Wikipedia and BookCorpus — the same data used for BERT — giving it strong general language representations out of the box. For an 8-class genre classification task on Goodreads reviews, DistilBERT provides the right trade-off between accuracy and computational efficiency, making it the most practical choice for this MLOps assignment.
"""
...
# 1. Re-initialize the Trainer with your loaded model and dataset
eval_trainer = Trainer(
    model=trainer,  # Pass the loaded model variable here
    args=training_args,
    eval_dataset=test_dataset
)

# 2. Now call predict on the Trainer object
predicted_results = eval_trainer.predict(test_dataset)
...
preds  = eval_trainer.predict(test_dataset).predictions.argmax(-1)
...
model.push_to_hub("sharvanvittala007/distilbert-goodreads-genres")
tokenizer.push_to_hub("sharvanvittala007/distilbert-goodreads-genres")

# Record the link in W&B
wandb.run.summary["huggingface_model"] = \
    "https://huggingface.co/sharvanvittala007distilbert-goodreads-genres"
wandb.finish()
print("finished.")
